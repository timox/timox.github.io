...
// === FONCTIONS DE DEBUG ET TESTS ===
function debugGristUser() {
  if (!window.kanbanManager) {
    console.log('KanbanManager non initialisé');
    return;
  }
  
  console.log('=== TEST UTILISATEUR GRIST ===');
  
  const userName = window.kanbanManager.currentUser;
...
async function testUserSystem() {
  ...
  await window.kanbanManager.initializeUser();
  
  // 2. Afficher le résultat
  console.log('\n2. Résultat:');
  if (window.kanbanManager.currentUser) {
    console.log(`✅ Utilisateur: ${window.kanbanManager.currentUser}`);
  } else {
  ...
  console.log('\n3. Test timestamp...');
  const testDescription = window.kanbanManager.addTimestampToDescription('', 'Test de commentaire', null);
  console.log('Résultat timestamp:', testDescription);
  
  // 4. Vérifier les colonnes Grist
  console.log('\n4. Colonnes utilisateur Grist:');
  console.log('- last_modified_by:', window.kanbanManager.availableColumns.has('last_modified_by') ? '✅' : '❌');
  console.log('- last_modified_at:', window.kanbanManager.availableColumns.has('last_modified_at') ? '✅' : '❌');
  
  // 5. Tester la table User_Actions
  console.log('\n5. Test table User_Actions...');
  try {
    await window.kanbanManager.logUserAction(999, 'test', 'ancienne_valeur', 'nouvelle_valeur', 'Test système');
  ...
  return {
    userDetected: !!window.kanbanManager.currentUser,
    userName: window.kanbanManager.currentUser,
    columnsOk: window.kanbanManager.availableColumns.has('last_modified_by'),
    testComplete: true
  };
}

async function showUserActions(taskId = null) {
  if (!window.kanbanManager) {
    console.log('KanbanManager non initialisé');
    return;
  }
  ...
  try {
    const userActions = await grist.docApi.fetchTable(window.kanbanManager.USER_ACTIONS_TABLE);
    ...
    const actions = window.kanbanManager.mapGristRecords(userActions)
...
async function diagnosticUserSystem() {
  if (!window.kanbanManager) {
    console.log('KanbanManager non initialisé');
    return;
  }
  ...
  const hasUserColumns = window.kanbanManager.availableColumns.has('last_modified_by');
  console.log(`${hasUserColumns ? '✅' : '❌'} Colonnes utilisateur détectées`);
  
  if (window.kanbanManager.currentRecords && hasUserColumns) {
    const users = [...new Set(window.kanbanManager.currentRecords
...
function clearAllDates() {
  ...
  if (confirm('Supprimer toutes les dates butoir de toutes les tâches ?')) {
    window.kanbanManager.currentRecords.forEach(async (record) => {
      if (record.date_echeance) {
        try {
          await grist.docApi.applyUserActions([
...
window.KanbanDebug = {
  ...
  showColumns: () => {
    if (window.kanbanManager) {
      ...
      console.log(Array.from(window.kanbanManager.availableColumns).sort());
    }
  },

  // Afficher les données brutes d'une tâche
  showRawTask: (taskId) => {
    if (window.kanbanManager) {
      const task = window.kanbanManager.currentRecords.find(r => r.id === taskId);
      if (task) {
        console.log('=== DONNÉES BRUTES TÂCHE ===');
        console.log(task);
      } else {
  ...
  // Tester le système de dates
  testDates: (taskId) => {
    if (window.kanbanManager && taskId) {
      window.kanbanManager.debugDates(taskId);
    }
  },

  // Afficher rapport complet d'une tâche
  fullReport: (taskId) => {
    if (window.kanbanManager && taskId) {
      window.kanbanManager.showTaskCompleteReport(taskId);
    }
  },
  ...
  systemCheck: () => {
    ...
    console.log('KanbanManager initialisé:', !!window.kanbanManager);
    if (window.kanbanManager) {
      console.log('Nombre de tâches:', window.kanbanManager.currentRecords.length);
      console.log('Mode de vue:', window.kanbanManager.viewMode);
      console.log('Utilisateur:', window.kanbanManager.currentUser || 'Non détecté');
      console.log('Modal initialisée:', !!window.kanbanManager.modal);
      console.log('DatePicker initialisé:', !!window.kanbanManager.datePickerManager);
    }
  },
  ...
  reloadData: async () => {
    if (window.kanbanManager) {
      try {
        await window.kanbanManager.loadGristDataAndOptions();
        window.kanbanManager.refreshKanban();
  ...
  reset: () => {
    if (window.kanbanManager) {
      window.kanbanManager.expandedCards.clear();
      window.kanbanManager.filters = { bureau: '', qui: '', projet: '', statut: '' };
      window.kanbanManager.showTermine = true;
      window.kanbanManager.viewMode = 'compact';
      window.kanbanManager.refreshKanban();
...
// === RACCOURCIS DE CONSOLE POUR DEBUG ===
console.log('🔧 KANBAN DEBUG DISPONIBLE :');
console.log('- KanbanUtils.testUserSystem() : Test système utilisateur');
console.log('- KanbanUtils.showBureauStats() : Statistiques par bureau'); 
console.log('- KanbanDebug.systemCheck() : Vérification système');
